...
- ServiceImpl 中只要出现多分支、跨表写入、唯一性校验、状态变更、外部调用、缓存/消息/任务等稍复杂业务逻辑，必须在代码块前写业务逻辑注释。
- 简单 CRUD、字段赋值、单行转换和直接调用可不写注释；不要给每一行机械补注释。
...
- ServiceImpl 中稍复杂业务代码块是否写清业务规则、顺序依赖和影响范围。
